...
class ChatGPTAutomation:
    ...
    def read_conversations_into_csv(self, num_conversations):
        chat_elements = self.return_chat_elements()
        for i in reversed(range(1, num_conversations)):
            chat_elements = self.return_chat_elements()
            chat_elements[i].click()
    ...
    def write_to_conversations(self, queries_per_conversation):
        try:
            cur_queries = 1
            self.num_conversations = 1

            while cur_queries < 400:
                for row in self.data:
                    ...
                    if len(row) > 4 and (row[4] == 'GPT 4 RAG' or not row[4].strip() or row[5] == '[]'):
                        ...
                        while self.is_button_present("//button[@aria-label='Stop generating']"):
                            time.sleep(20)
                        print("Got response. Sleeping to load.")
                        time.sleep(35)
            ...
            for i in (reversed(range(0, self.num_conversations))):
                chat_elements = self.return_chat_elements()
                chat_elements[i].click()
                print("Loading chat to read into .csv file. Index: ", i)
                time.sleep(30)
    ...
    def populate_conversations(self, queries_per_conversation, max_num_conversations):
        ...
        try:
                ...
                for row in self.data:
                    ...
                    if num_conversations == max_num_conversations:
                        for i in reversed(range(1, max_num_conversations)):
                            ...
                            conversation = self.return_chatgpt_conversation()
                            self.process_conversation(conversation)
                            print("Wrote current conversation to csv.")
                            # self.open_new_conversation()
                        print("Done writing to .csv. Opening new conversation.")
                        self.open_new_conversation()
                        num_conversations = 1
                        cur_queries = 0
            ...
            print("Ran into unexpected condition. Reading current results into .csv and closing session.")
            for i in reversed(range(1, max_num_conversations)):
                        chat_elements = self.return_chat_elements()
                        chat_elements[i].click()
                        print("Loading chat to read into .csv file. Index: ", i)
